   ...
   tuneLength = 3)
{
   ...
   loadPro <- try(library(nwsPro), silent = TRUE)
   if(class(loadPro) == "try-error")
   {
      hasPro <- FALSE
      require(nws)
    } else hasPro <- TRUE
   ...
   for(i in seq(along = tuneGrid[,1]))
   {
      ...
      # for non-oob error rates, we will fit the model and return the held-out prediction
      if(trControl$method != "oob")
      {
         ...
         if(hasPro)
         {
            resampleMatrix <- do.call(
                  cbind,
                  nwsPro:::eachElemFT(
                     sleighObj,
                     resampleWrapper,
                     list(ind = trControl$index),
                     list(x = argList),
                     eo = list(tvalue = NULL)))
         } else {
            resampleMatrix <- do.call(
